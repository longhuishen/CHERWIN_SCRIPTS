...
function die {
    ...
    echo -e "$prefix: "
    echo -e "$prefix: ${bold}You can contact Moonlight Game Streaming for further assistance.${reset}"
    echo -e "$prefix: "
    
    echo -e "$prefix: ${bold}URL: https://myqunhui.eu.org/https://github.com/moonlight-stream/moonlight-qt${reset}"
    echo -e "$prefix: "
...
function config_url {
    ...
    echo "https://dl.cloudsmith.io/public/moonlight-game-streaming/moonlight-qt/config.deb.txt?$params" | sed 's/ /%20/g'
}
...
EOF
 ...
 >>>>: doesn't work after an hour, please contact Moonlight Game Streaming
 >>>>: for assistance.
 >>>>:
EOF
        }
...
function import_gpg_key {
    local text="Importing 'moonlight-game-streaming/moonlight-qt' repository GPG keys ..."
    echo_running "$text"

    local gpg_keyring_path="/usr/share/keyrings/moonlight-game-streaming-moonlight-qt-archive-keyring.gpg"
    
    curl -1sLf "https://dl.cloudsmith.io/public/moonlight-game-streaming/moonlight-qt/gpg.2F6AE14E1C660D44.key" | gpg --dearmor >> $gpg_keyring_path
    ...
    echo_okfail "Checking for apt signed-by key support ..." || {
        mv ${gpg_keyring_path} /etc/apt/trusted.gpg.d/moonlight-game-streaming-moonlight-qt.gpg
        chmod 644 /etc/apt/trusted.gpg.d/moonlight-game-streaming-moonlight-qt.gpg
    }
...
function setup_repository {
    local repo_path="/etc/apt/sources.list.d/moonlight-game-streaming-moonlight-qt.list"
    check_fetch_config

    local text="Installing 'moonlight-game-streaming/moonlight-qt' repository via apt ..."
    echo_running "$text"
...
echo "Executing the  setup script for the 'moonlight-game-streaming/moonlight-qt' repository ..."
echo
